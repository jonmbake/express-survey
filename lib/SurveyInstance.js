...
_.extend(SurveyInstance.prototype, {
   ...
   */
  toJSON: function (includeNameField) {
    var fields;
    if (includeNameField && this.invite_list) {
      fields = [{ title: 'Name', name: '_name', type: 'text'}].concat(this.fields);
    } else {
      fields = this.fields;
    }
    return _.extend({fields: fields}, _.pick(this, 'title', 'name', 'url', 'message', 'html_escape_message'));
  }
});
