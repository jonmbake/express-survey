...
_.extend(SecuredResponseDatabase.prototype, ResponseDatabase.prototype, {
  ...
  queryAll: function (sortBy, sortDir, token) {
    return this.hasAccess(token).then(function () {
      var deferred = Q.defer();
      this.db.all('select i.name "_name", i.response_id, ' + this._columnNames() + ' from invitees i left outer join responses r on (r.id = i.response_id) order by ' + (sortBy + 1)  + ' ' + sortDir, function(err, results) {
        if (err) {
          deferred.reject(err);
        } else {
